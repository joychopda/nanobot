...
description: "Manage a Paperclip company as a board member via chat. Covers onboarding (company creation, CEO setup, hiring plans), agent management, approvals, task monitoring, cost oversight, and work product review. Use whenever the user wants to interact with their Paperclip control plane."
...
1. Check if `PAPERCLIP_API_URL` is set.
   - If not set and the user needs to install or start Paperclip: read [references/setup-installation.md](references/setup-installation.md).
